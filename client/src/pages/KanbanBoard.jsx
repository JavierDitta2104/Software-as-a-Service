...
import { listService, cardService, activityService } from '../services/api';
import { Plus, MoreVertical, Calendar, Clock, Activity, X, Tag, Trash2 } from 'lucide-react';
...
const KanbanBoard = () => {
    ...
    const [newCardData, setNewCardData] = useState({ title: '', description: '', priority: 'low' });
    ...
    const handleAddCard = async (e) => {
        ...
        try {
            ...
            setNewCardData({ title: '', description: '', priority: 'low' });
    ...
    return (
        <DragDropContext onDragEnd={onDragEnd}>
            <div style={{ position: 'relative', display: 'flex', height: 'calc(100vh - 100px)', overflow: 'hidden' }}>
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div className="glass" style={{ width: '500px', padding: '2rem', borderRadius: '1rem', background: 'var(--bg-card)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                            ...
                            <X size={20} style={{ cursor: 'pointer', color: 'var(--text-muted)' }} onClick={() => setActiveListIdForCard(null)} />
                        </div>
